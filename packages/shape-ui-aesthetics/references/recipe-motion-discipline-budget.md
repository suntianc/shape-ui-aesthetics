...
**Data provenance (updated 2026-07-26):** duration bands below come from a per-element computed-style survey across seven reference sites (element-by-element histograms of `transition-duration`, easing, and animation duration), replacing an earlier capture that had frozen all durations. Every band in the table is now directly measured.
...
| Transition duration tiers | Micro-feedback 0.1–0.2s (the dominant tier on every site measured); container/composite 0.3–0.4s; tool-site ceiling about 0.5s; the 0.6–0.75s narrative tier belongs to brand/portfolio sites only | Tool controls above 0.5s; more than 4 common duration values on one site (scattered magic numbers) | Measured histograms (2026-07-26): Linear 0.16s x112; Family 0.2s x193 + 0.1s x94; Raycast 0.2s x590 + 0.4s x506; iA 0.2s x45; Amie 0.15s x10; landonorris.com 0.75s x118 |
| Duration convergence | 2–4 common values cover roughly 95% of transition usage per site (tiers derived from one token) | A magic-number duration per animation site | All seven measured histograms converge; the landonorris.com token `--duration-default: 0.75s` matches its measured x118 usage |
| Signed-curve usage share | One primary curve carries the overwhelming majority of a site's custom easing usage | Several curves mixed evenly | Measured: Raycast `(0.23,1,0.32,1)` x980; Linear `(0.25,0.46,0.45,0.94)` x122; landonorris.com `(0.65,0.05,0,1)` x118; Amie and Emil Kowalski share the standard curve `(0.4,0,0.2,1)` across sites |
...
- **Note (gap closed 2026-07-26):** the earlier unverifiable millisecond band has been re-measured per element and promoted into the section 5 bands. The comparison is instructive: convention (100–300ms) roughly matched the measured dominant tier (100–200ms) but underestimated the real weight of the 0.3–0.4s container tier (Raycast uses 0.4s five hundred times) — a direct case of measured exemplars beating remembered convention.
